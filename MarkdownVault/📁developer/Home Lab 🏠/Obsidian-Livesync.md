...
## installation
### backend
1. spin up a [CouchDB](📁developer/Home%20Lab%20🏠/CouchDB.md) server and make it public facing via [Nginx Proxy Manager](📁developer/Home%20Lab%20🏠/Nginx%20Proxy%20Manager.md). **vrtmrz** also has an [all in one solution](https://github.com/vrtmrz/self-hosted-livesync-server) if your are not currently using a **Reverse Proxy**
2. `./compose.yml`
```yaml
version: "3.7"
...
> **Obsidian-livesync** only syncs when 2 client instances of Obsidian are open *(i.e. the mobile app and desktop app)*. I like to think of [vrtmrz/filesystem-livesync](https://github.com/vrtmrz/filesystem-livesync) as the #headless version of [Obsidian.md](📁developer/Home%20Lab%20🏠/Obsidian.md.md) that makes sure all the other are up-to-date
